<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html>
<head>
<link href="https://stackpath.bootstrapcdn.com/bootswatch/4.3.1/superhero/bootstrap.min.css" rel="stylesheet" integrity="sha384-LS4/wo5Z/8SLpOLHs0IbuPAGOWTx30XSoZJ8o7WKH0UJhRpjXXTpODOjfVnNjeHu" crossorigin="anonymous">
<meta charset="ISO-8859-1">
<title>spaceship-selection</title>
</head>
<body>
<a class="btn btn-primary" href="spaceship" id="bb">Blue Blazer</a>
<a class="btn btn-primary" href="spaceship" id="gg">Grass Glider</a>
<a class="btn btn-primary" href="spaceship" id="ff">Folk Flyer</a>
</body>
</html>